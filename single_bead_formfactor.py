    ...
    with open('martini_atomistic.csv', 'w', newline='') as csvfile:
        ...
        for restype in restype_1to3.keys():
            ...
            if calculate_martini:
                ...
                print(beads_idx)
                #add weights such that atoms are not counted more than once
                weights = []
                for bi, b in enumerate(beads_idx):
                    bead_weights = []
                    for idx in b:
                        count = sum(idx in sublist for sublist in beads_idx)
                        bead_weights.append(1.0 / count)
                    weights.append(bead_weights)
                print(weights)
            else: # single bead
                n_beads = 1
                beads = ["AA"]
            ...
            for i, crd in tqdm.tqdm(enumerate(all_coordinates_non_hydrogen), total=n_rotamer):
                if calculate_martini:
                    for bi, b in enumerate(beads):
                        ...
                        w = np.array(weights[bi])[:, None]
                        F[bi, i] = getAAFormFactor_fast(dgram, q, w * ff_excl[beads_idx[bi]])
            ...
            for b in range(n_beads):
                ...
                coeffs = polynomial.keywords['coeffs']
                for i, coeff in enumerate(coeffs):
                    writer.writerow([restype3, beads[b], i, coeff])
...
if __name__ == "__main__":
    ...
    parser.add_argument( "--martini_mapping_path", type=str,help="", default="./martini3001")
